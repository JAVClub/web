<template>
    <div>
        ...
        <waterfall :line-gap="320" :align="'center'" :watch="items">
            ...
            :width="320"
            >
            <a :href="`/#/metadata/list/1/series/${item.id}`"><a-card style="width: 310px">
                {{item.name}}
            </a-card></a>
            </waterfall-slot>
        </waterfall>
...
<script>
    ...
    export default {
        ...
        methods: {
            requestNew: function() {
                this.axios.get(this.apiHost + `/metadata/getMetaList/series/${this.page}/${this.pageSize}`).then((res) => {
                    res = res.data
            ...
            pageChange: function(data) {
                ...
                this.$router.push(`/series/list/${this.page}`)
                this.requestNew()
            },
        },

        created: function() {
            document.title = 'Series List | JAVClub'
